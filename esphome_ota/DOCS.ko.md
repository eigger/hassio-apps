...
토큰이 없던 기존 기기가 새로운 보안 펌웨어를 처음 수신하거나 토큰을 재생성할 때, 애드온은 구버전 경로(`livingroom.json`)와 신규 토큰 경로(`livingroom_<token>.json`) **양쪽에 파일을 동시에 게시**합니다.

기기가 새 버전 설치를 성공적으로 마치거나(또는 자동 숨김 타이머가 만료되면), 애드온이 **브릿지를 자동으로 철거**합니다:
1. `/local/` 및 스토리지에서 비토큰 레거시 파일(`{node}.json`, `{node}.ota.bin` 등)을 즉시 영구 삭제합니다.
2. 레지스트리의 `legacy_bridge`를 비활성화(`False`)하여 이후 게시부터는 안전한 토큰 URL로만 배포합니다.
3. 0.8.0+ 버전에서 새로 등록하는 기기는 레거시 브릿지를 거치지 않고 처음부터 토큰 URL만 사용합니다.
