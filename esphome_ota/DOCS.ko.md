...
### 왜 시크릿 토큰 슬러그를 사용하나요?
Home Assistant의 `/local/` 정적 파일 경로는 기본적으로 인증 없이 열려 있으며, 폴더 목록 조회는 차단(`404 Not Found`)되어 있습니다. 파일명에 128비트 암호학적 난수 토큰을 붙여 저장함으로써, 외부 해커나 자동화 봇이 파일명을 추측(Brute-force / 사전 공격)하여 Wi-Fi 비밀번호나 API 암호화 키를 추출하는 것을 방어합니다.

> [!NOTE]
> **보안 범위 및 한계점**:
> 시크릿 토큰 URL은 컴파일된 펌웨어 바이너리 내부에 일반 문자열 리터럴로 포함됩니다. 공격자가 기기 플래시에 물리적으로 접근하거나 이미 배포된 `.bin` 파일을 확보하여 디컴파일하는 경우 토큰 문자열이 노출될 수 있습니다. 토큰 슬러그의 주 목적은 공개된 `/local/` 엔드포인트에 대한 **외부 무차별 대입 스캔을 방어**하는 것입니다. 만약 토큰 유출이 의심되는 경우, 대시보드에서 **토큰 재생성** 버튼을 눌러 이전 파일을 즉시 무효화하고 새로운 비밀 URL을 발급할 수 있습니다.

### 무중단 1회성 전환 브릿지 (Dual-Publish Bridge)
토큰이 없던 기존 기기가 새로운 보안 펌웨어를 처음 수신하거나(None → Token B), 기존 토큰을 쓰던 기기가 토큰을 새로 재생성할 때(Token A → Token B), 원격 기기는 여전히 자신이 이전에 알고 있던 **이전 주소(Previous URL)**를 폴링하고 있습니다.

기기 고립(OTA 불가)을 방지하기 위해, 애드온은 새 펌웨어 배포 시 **이전 주소(구버전 또는 Token A)**와 **새 주소(Token B)** 양쪽에 파일을 **1회 동시 배포(Dual-Publish)**합니다.

기기가 이전 주소로 접속해 새 토큰이 심어진 펌웨어를 받아 설치를 완료하면(또는 자동 숨김 타이머가 만료되면), 애드온이 **브릿지를 자동으로 철거**합니다:
1. `/local/` 및 스토리지에서 이전 주소 파일(`{node}.*` 또는 `{node}_{prev_token}.*`)을 즉시 영구 삭제합니다.
2. 레지스트리의 `previous_token`을 초기화(`null`)하여 이후 게시부터는 안전한 신규 토큰 URL로만 단독 배포합니다.
3. 0.8.0+ 버전에서 새로 등록하는 기기는 브릿지 없이 처음부터 신규 토큰 URL만 사용합니다.
