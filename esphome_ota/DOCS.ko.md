...
#### 4. 업데이트 미진행 시 바이너리 비활성화 (숨김)
OTA 업데이트를 진행할 때 펌웨어를 활성화/배포하고, 원격 기기들의 업데이트가 완료되면 해당 행의 **`비활성화 (숨김)`** 버튼을 누르세요:
- Home Assistant의 `/local/` 경로에서 `.bin` 바이너리가 즉시 제거되어 평상시 외부 인터넷으로의 자격증명 노출을 차단합니다.
- 바이너리는 애드온 내부 전용 저장소(`/data/firmware`)에 안전하게 보관되므로, 추후 다른 기기를 업데이트하거나 재배포할 때 다시 파일을 올릴 필요 없이 **`활성화 (공개)`** 버튼 클릭 한 번으로 즉시 복원할 수 있습니다.
- `/local/`의 `.json` 매니페스트는 유지되므로 기기들의 주기적 업데이트 확인 시 404 에러 없이 정상 응답합니다.
