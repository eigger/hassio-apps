...
# ==============================================================================
# Drop app: prepare persistent data, secrets, and Postgres
# ==============================================================================
set -euo pipefail
...
bashio::log.info "Preparing Drop data directories"
...
if [[ ! -f "${SECRETS_FILE}" ]]; then
  ...
  umask 077
  {
    echo "POSTGRES_USER=drop"
    echo "POSTGRES_DB=drop"
...
# The chunk staging and thumbnail directories are derived from it, so they land on the
# same filesystem and finished uploads are moved into place with a rename().
cat > "${RUNTIME_ENV}" <<ENV
